...
public class BossBulletSpreader : BossShooter 
{
	...
	protected override void Update () 
	{
		...
		if (mTimerTemp <= 0) 
		{
			...
			bulletOne.GetComponent<EnemyBulletController>().mFireDir = Quaternion.Euler(0f,0f,30f) * Vector3.Normalize(mBossCentral.mTargetedPlayer.transform.position - transform.position);
			...
			bulletTwo.GetComponent<EnemyBulletController>().mFireDir = Quaternion.Euler(0f,0f,-30f) * Vector3.Normalize(mBossCentral.mTargetedPlayer.transform.position - transform.position);
